...
from django.contrib.auth import get_user_model, login as login_builtin, authenticate, logout as logout_builtin, update_session_auth_hash
...
def _already_logged_in_redirect(request):
    if request.user.is_authenticated:
        ...
        # I don't think we need the next two lines since the login/signup POST forms will have invalid CSRF tokens if the user has logged in.
...
def _signup_or_login(request, signup_or_login):
    if signup_or_login == 'signup':
        ...
        raise ValueError('Unrecognized option. Use "signup" or "login".')
